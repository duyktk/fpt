...
public class SearchCategoryController extends HttpServlet {
    ...
    private final String homeauth = "homeauth.jsp";
    /**
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String category = request.getParameter("txtSearch");
        String url = home;
        DAO dao = new DAO();
        Cookie[] cookies = request.getCookies();
        boolean check = false;
        for(Cookie cookie : cookies){
            check = dao.checkLogin(cookie.getName(), cookie.getValue());
        }
        if(category!=null){
            dao.searchByCategory(category);
            List<CourseRegistration> result = dao.getCourseList();
            request.setAttribute("SEARCHRESULT", result);
        }
        if(check) url = homeauth;
